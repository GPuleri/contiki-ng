...
PROCESS(project1_3, "project1_3");
AUTOSTART_PROCESSES(&project1_3);
/*---------------------------------------------------------------------------*/
 ...
 *
 */
PROCESS_THREAD(project1_3, ev, data) {
   ...
   tsch_set_coordinator(1); // set the node as coordinator
   ...
   while (1) {
      ...
      if (rx_count > 0) { // check if the leaf node has joined the network
