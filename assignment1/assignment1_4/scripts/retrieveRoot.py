...
mycolumns=['CPU (s)','LPM (s)','LISTEN (s)','Total time (s)',\
                       ...
                       'Total_energy (J)','Latency (ms)','Throughput (%)','kbit/Joule']
...
pattern2=r'Received from 0012.4b00.1932.e489 with latency of (\d+)ms*[\n]'

file=open(inFilename,'r').read()
stats=re.findall(pattern,file)
stats2=re.findall(pattern2,file)
...
for values in stats:
	...
	for values2 in stats2:
		...
		data=[[values[0],values[1],values[4],values[3],\
			...
			Latency,Packet_received,((0.064*int(Packet_received))/Total_energy)]]
